<template>
  <div>
    ...
    <div v-if="priceSelected == false" class="price-box">
      <span v-if="product.prices != undefined" class="product-price"
        >from {{ currency }}{{ addCommas(product.prices[0].price) }}</span
      >
    </div>
    <!-- End .price-box -->
    ...
    <div v-else class="price-box">
      <div v-if="priceSelected.is_discount">
        <span class="old-price"
          >{{ currency }}{{ addCommas(priceSelected.price) }}</span
        >
        <span class="product-price"
          >{{ currency }}{{ addCommas(priceSelected.discounted_price) }}</span
        >
      </div>
      <div v-else>
        <span class="product-price"
    ...
    <div
      ...
      v-if="product.colors != undefined && product.colors.length > 0"
    >
      <div class="product-single-filter">
        ...
        <ul class="config-swatch-list">
          <li
            v-bind:class="{
              active:
                attributesChosen.color != undefined &&
                attributesChosen.color == colorEntry.name,
            }"
            v-for="colorEntry in product.colors"
            :key="colorEntry.id"
          >
            <p>color is: {{ attributesChosen.color }}</p>
            <a
              @click="chooseColor(colorEntry.name)"
              :style="'background-color: ' + colorEntry.code"
            ></a>
          </li>
        </ul>
      </div>
    ...
    >
      <div class="product-single-filter">
        ...
        <div>
          <a
            @click="attributesChosen[attribute.name] = attributeEntry.value"
            v-bind:class="[
              attributesChosen[attribute.name] == attributeEntry.value
                ? 'btn-primary'
                : '',
              'btn-default',
            ]"
            ...
            style="
              ...
              margin: 0 10px;
            "
            v-for="attributeEntry in attribute.value"
            :key="attributeEntry.id"
            >{{ attributeEntry.value }}</a
          >
        </div>
      </div>
      <!-- End .product-single-filter -->
    ...
    <div class="product-action">
      ...
      <a
        ...
        v-if="priceSelected == false"
        class="paction add-cart"
        title="Add to Cart?"
        @click="showError('You have to select colors and/or types')"
      >
...
export default {
  ...
  methods: {
    ...
    showError(errorMessage) {
      this.$swal("Oops!", errorMessage, "error");
    },
    chooseColor(color) {
      const self = this;
    ...
    getProductInfo(productId) {
      ...
      axios
        .get("/json/product/detail/" + self.product_id)
        .then((response) => {
    ...
    isProductSelected() {
      ...
      if (self.product.prices != undefined) {
        self.product.prices.forEach(function (entry) {
          //alert('nice');
          if (
            JSON.stringify(attributeToCompare.sort()) ===
            JSON.stringify(entry.keywordsArray.sort())
          ) {
            //Vue.set(self.attributesChosen, entry.name, 'none');
